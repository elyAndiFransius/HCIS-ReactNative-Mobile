...
function Jadwal() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      // === UBAH KE [] untuk lihat tampilan kosong ===
      setJadwalList([
        {
          id: 1,
          nama: 'Kevin Abas Surya',
          no_rm: '1202349812023498',
          kode_booking: '20250902070908',
          tanggal: 'Senin, 17 Sept 2025',
          qr_image: 'https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=KevinAbas',
        },
        {
          id: 2,
          nama: 'Andre Vicky',
          ...
          kode_booking: '20250902070908',
          tanggal: 'Senin, 17 Sept 2025',
          qr_image: 'https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=AndreVicky',
        },
        {
          id: 3,
          nama: 'Cecilia Lauren',
          ...
          kode_booking: '20250902070908',
          tanggal: 'Senin, 17 Sept 2025',
          qr_image: 'https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=CeciliaLauren',
        },
      ]);
      setLoading(false);
    }, 1000);
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      {loading ? (
        ...
        <View className="flex-1 justify-center px-5">
          <View className="justify-center items-center shadow py-10 px-5 bg-slate-200 rounded-lg">
            <Image
              source={require('../../assets/images/PagerView/pagerView3-removebg.png')}
              className="w-52 h-52"
            />
          ...
          <View className="flex-row bg-slate-200 rounded-xl mt-4 p-4 shadow-md items-center">
            <View className="w-20 h-20 rounded-lg bg-blue-100 justify-center items-center">
              <Text className="text-blue-600 font-bold">E-Jadwal</Text>
            </View>
            <View className="flex-1 ml-3">
        ...
        <FlatList
          ...
          contentContainerStyle={{ padding: 16 }}
          renderItem={({ item }) => (
            <View className="flex-row justify-between items-center bg-white p-4 mb-3 rounded-xl shadow">
              <View className="flex-1 mr-3">
                <Text className="text-blue-900 font-bold text-base">
                  Nama : {item.nama}
                </Text>
                <Text className="text-gray-700">No RM : {item.no_rm}</Text>
                <Text className="text-gray-700">K. Booking : {item.kode_booking}</Text>
              </View>

              <View className="bg-gray-100 px-3 py-2 rounded-lg items-center">
                <Text className="text-gray-800 font-semibold text-sm mb-1">QR Check in</Text>
                <Image source={{ uri: item.qr_image }} className="w-12 h-12" />
                <Text className="text-gray-500 text-xs mt-1">{item.tanggal}</Text>
              </View>
            </View>
          )}
        />
